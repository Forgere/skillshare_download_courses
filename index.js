...
function main(url) {
  async.waterfall([
    ...
    (accountId, videos, direction, callback) => {
      async.parallelLimit(videos.map((videoId) => cb => {
        const wr = child_process.fork('src/downVideo.js')
        wr.send([accountId, videoId, `./${direction}`])
